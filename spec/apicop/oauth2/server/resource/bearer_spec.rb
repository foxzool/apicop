...
describe APICop::OAuth2::Server::Resource::Bearer do
  let(:app) do
    APICop::OAuth2::Server::Resource::Bearer.new(simple_app) do |request|
  ...
  let(:bearer_token) do
    APICop::OAuth2::AccessToken::Bearer.new(:access_token => 'valid_token')
  end
  ...
  context 'when invalid_token is given' do
    ...
    describe 'realm' do
      ...
      context 'when specified' do
        ...
        let(:app) do
          APICop::OAuth2::Server::Resource::Bearer.new(simple_app, realm) do |request|
      ...
      context 'otherwize' do
        it 'should use default realm' do
          status, header, response = request
          expect(header['WWW-Authenticate']).to include "Bearer realm=\"#{APICop::OAuth2::Server::Resource::Bearer::DEFAULT_REALM}\""
        end
      end
    end
  end
