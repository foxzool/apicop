...
describe APICop::OAuth2::Server::Resource::Unauthorized do
  let(:error) { APICop::OAuth2::Server::Resource::Unauthorized.new(:invalid_token) }
  let(:realm) { APICop::OAuth2::Server::Resource::DEFAULT_REALM }
  ...
  context 'when scheme is defined' do
    ...
    describe '#finish' do
      ...
      context 'when error_code is not invalid_token' do
        let(:error) { APICop::OAuth2::Server::Resource::Unauthorized.new(:something) }
      ...
      context 'when realm is specified' do
        ...
        let(:error) { APICop::OAuth2::Server::Resource::Bearer::Unauthorized.new(:something, nil, :realm => realm) }
...
describe APICop::OAuth2::Server::Resource::Forbidden do
  let(:error) { APICop::OAuth2::Server::Resource::Forbidden.new(:insufficient_scope) }

  it { should be_a APICop::OAuth2::Server::Abstract::Forbidden }
  ...
  context 'when scope option is given' do
    let(:error) { APICop::OAuth2::Server::Resource::Bearer::Forbidden.new(:insufficient_scope, 'Desc', :scope => [:scope1, :scope2]) }
...
describe APICop::OAuth2::Server::Resource::Bearer::ErrorMethods do
  ...
  APICop::OAuth2::Server::Resource::ErrorMethods::DEFAULT_DESCRIPTION.keys.each do |error_code|
    method = "#{error_code}!"
    case error_code
